...
class MeteoMetadataProvider(MetadataProvider):
    ...
    def fetch_metadata(self, station_ids):
        ...
        station_ids = [str(sid).strip() for sid in station_ids if sid and str(sid).strip()]
        ...
        if missing:
            try:
                ...
                with self.engine.connect() as conn:
                    ...
                    self.logger.debug(f"Fetched metadata for {len(self._cache) - len(cached)} new stations")
